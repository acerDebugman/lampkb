# Ported in full from graphify/tests/test_querylog.py.
# graphify -> kglib; env vars adapted GRAPHIFY_QUERY_LOG* -> KG_QUERY_LOG*.
# Note: the default enable-flag log path is "~/.cache/kg-queries.log"
# (renamed from upstream's graphify-queries.log as kg branding).
import json
import pytest
from pathlib import Path

from kglib.querylog import log_query, nodes_from_result
...
def test_log_query_writes_jsonl(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("KG_QUERY_LOG", str(log_file))
    monkeypatch.delenv("KG_QUERY_LOG_DISABLE", raising=False)
...
def test_disable_env(tmp_path, monkeypatch):
    log_file = tmp_path / "q.log"
    monkeypatch.setenv("KG_QUERY_LOG", str(log_file))
    monkeypatch.setenv("KG_QUERY_LOG_DISABLE", "1")
...
def test_disable_env_true(tmp_path, monkeypatch):
    log_file = tmp_path / "q.log"
    monkeypatch.setenv("KG_QUERY_LOG", str(log_file))
    monkeypatch.setenv("KG_QUERY_LOG_DISABLE", "true")
...
def test_responses_not_logged_by_default(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("KG_QUERY_LOG", str(log_file))
    monkeypatch.delenv("KG_QUERY_LOG_DISABLE", raising=False)
    monkeypatch.delenv("KG_QUERY_LOG_RESPONSES", raising=False)
...
def test_responses_optin(tmp_path, monkeypatch):
    log_file = tmp_path / "q.log"
    monkeypatch.setenv("KG_QUERY_LOG", str(log_file))
    monkeypatch.setenv("KG_QUERY_LOG_RESPONSES", "1")
    monkeypatch.delenv("KG_QUERY_LOG_DISABLE", raising=False)
...
def test_log_never_raises(tmp_path, monkeypatch):
    ...
    bad_path.mkdir()
    monkeypatch.setenv("KG_QUERY_LOG", str(bad_path))
    monkeypatch.delenv("KG_QUERY_LOG_DISABLE", raising=False)
...
def _clear_log_env(monkeypatch):
    for k in ("KG_QUERY_LOG", "KG_QUERY_LOG_ENABLE", "KG_QUERY_LOG_DISABLE"):
        monkeypatch.delenv(k, raising=False)


def test_query_log_off_by_default(monkeypatch):
    from kglib.querylog import _log_path
...
def test_query_log_enabled_by_explicit_flag(monkeypatch):
    from kglib.querylog import _log_path
    _clear_log_env(monkeypatch)
    monkeypatch.setenv("KG_QUERY_LOG_ENABLE", "1")
    # kg branding: the default log filename was renamed from upstream's
    # "graphify-queries.log".
    assert str(_log_path()).endswith("kg-queries.log")


def test_query_log_enabled_by_explicit_path(monkeypatch, tmp_path):
    from kglib.querylog import _log_path
    _clear_log_env(monkeypatch)
    monkeypatch.setenv("KG_QUERY_LOG", str(tmp_path / "q.log"))
...
def test_log_query_writes_nothing_by_default(monkeypatch, tmp_path):
    ...
    assert not (tmp_path / ".cache" / "kg-queries.log").exists()
