...
| `test_build_ported.py` | `test_build.py` | Generic `build_from_json`/`build`/`build_merge` cases: dedupe helpers, fixture counts (against `tests/fixtures/extraction_docs.json`), weight normalization (#1960), legacy alias folds (#2194), id re-key (#1504/#2197/#2618), path handling (#932/#1279), entity_type canonicalization (#660/#840), semantic-tier ghost merges (#2068/#1753), directed-flag inheritance (#2342), bidirectional-pair direction (#1061), edge_data(s) (#796), malformed-input tolerance, doc-twin merge (#1799), prune-root recovery (#2446). Hyperedge member revalidation (#1916) dropped with the hyperedge mechanism — see the 2026-09-10 note below. Dropped as code/AST-only: AST-twin ghost merges, old-stem alias cases (C/C++ include fallback), cross-language phantom guards (#1749), two-tier AST/semantic build_merge tests, MCP id tests (#2408), and `test_build_merge_preserves_call_edge_direction` (imports the JS extractor). |
| `test_build_merge_ported.py` | `test_build_merge_shrink_guard.py`, `test_build_merge_hyperedges_and_prune.py` | #2497 identity shrink guard, #1796 replace-wins-over-delete. #1574 (hyperedge carry-over) dropped with the hyperedge mechanism — see the 2026-09-10 note below. Canned graph layout writes `kg-out/graph.json` instead of `graphify-out/graph.json` (`_infer_merge_root` keys on the out-dir name). |
