...
def test_file_hash_consistent(tmp_file):
    ...
    h1 = file_hash(tmp_file, cache_root=tmp_file.parent)
    h2 = file_hash(tmp_file, cache_root=tmp_file.parent)
    assert h1 == h2
...
def test_file_hash_changes(tmp_path):
    ...
    assert file_hash(f1, cache_root=tmp_path) != file_hash(f2, cache_root=tmp_path)
...
def test_cache_roundtrip(tmp_file, cache_root):
    ...
    save_cached(tmp_file, result, root=cache_root, cache_root=cache_root)
    loaded = load_cached(tmp_file, root=cache_root, cache_root=cache_root)
...
def test_cache_miss_on_change(tmp_file, cache_root):
    ...
    assert load_cached(tmp_file, root=cache_root, cache_root=cache_root) is None
...
def test_md_frontmatter_only_change_same_hash(tmp_path):
    ...
    f = tmp_path / "doc.md"
    f.write_text("---\nreviewed: 2026-01-01\n---\n\n# Title\n\nBody text.")
    h1 = file_hash(f, cache_root=tmp_path)
    f.write_text("---\nreviewed: 2026-04-09\n---\n\n# Title\n\nBody text.")
    h2 = file_hash(f, cache_root=tmp_path)
    assert h1 == h2
...
def test_save_semantic_cache_overwrites_by_default(tmp_path):
    ...
    save_semantic_cache([{"id": "b", "source_file": "doc.md"}], [], root=tmp_path, cache_root=tmp_path)
    cached = load_cached(f, root=tmp_path, kind="semantic", cache_root=tmp_path)
...
def test_save_semantic_cache_rejects_out_of_scope_source_file(tmp_path):
    ...
    assert saved == 1
    intended_cache = load_cached(intended, root=tmp_path, kind="semantic", cache_root=tmp_path)
