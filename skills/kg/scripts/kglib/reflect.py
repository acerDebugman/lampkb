...
"""Deterministic "work memory" reflection over kg-out/memory/.
...
and a given ``now``. When a graph (`graph.json` + `.kg_analysis.json`) is available
the lessons are also grouped by community label; without it they degrade to a single
flat section.

The artifact lands at ``kg-out/reflections/LESSONS.md`` rather than inside the wiki
...
from kglib.ingest import OUTCOMES
...
LEARNING_SIDECAR_NAME = ".kg_learning.json"
...
def _load_node_community(graph_path: Path, analysis_path: Path,
    ...
    Mirrors how `graphify export wiki` reads graph.json + .kg_analysis.json +
    .kg_labels.json. Community membership in the analysis sidecar is keyed by
...
def render_lessons_md(agg: dict[str, Any]) -> str:
    ...
    out: list[str] = [
        "# Lessons",
        "",
        f"_Auto-generated by `kg reflect` from {agg['total']} session "
        f"{'memory' if agg['total'] == 1 else 'memories'} in kg-out/memory/. "
...
def reflect(memory_dir: Path, out_path: Path,
    ...
    if graph_path is not None:
        ...
        analysis_path = Path(analysis_path) if analysis_path else (
            graph_path.parent / ".kg_analysis.json")
        labels_path = Path(labels_path) if labels_path else (
            graph_path.parent / ".kg_labels.json")
...
# graph.json as ``.kg_learning.json``. It carries which nodes have proven
...
def _resolve_source_path(src: str, graph_path: Path) -> Path | None:
    ...
    live in ``<root>/kg-out/`` (so its own dir is not the root) or directly
    at the root. Resolve the root in the most-likely order and return the first
    candidate where the file actually exists, so a defeated heuristic can never
    strand the file (every node would then look "changed"). The same search runs
    at write and read time, so the writer and reader resolve to the same file.

    Order: the layout-appropriate root *first* — graph.json's parent's parent
...
def write_learning_sidecar(agg: dict[str, Any], graph_path: Path,
    ...
    """Write ``.kg_learning.json`` next to ``graph_path`` deterministically.
