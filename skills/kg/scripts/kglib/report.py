...
def load_learning_for_report(graph_path) -> dict | None:
    ...
    Reads the ``.kg_learning.json`` overlay (preferred sources) next to
    ...
    try:
        gp = _Path(graph_path)
        from kglib.reflect import load_learning_overlay, load_memory_docs, aggregate_lessons
...
def generate(
    ...
    if built_at_commit:
        lines += [
            ...
            "- Run the kg update flow (`kg.py update-detect` / `update-merge`) after document changes.",
        ]
    ...
    if gap_count > 0 or amb_pct > 20:
        ...
        if thin_communities:
            lines.append(f"- **{len(thin_communities)} thin communities (<{min_community_size} nodes) omitted from report** — run `kg query` to explore isolated nodes.")
        if amb_pct > 20:
    ...
    # Preferred sources come from the .kg_learning.json sidecar; the
