...
def _log_path() -> Path | None:
    ...
    #   KG_QUERY_LOG=<path>   log to that path, or
    #   KG_QUERY_LOG_ENABLE=1 log to ~/.cache/kg-queries.log.
    # KG_QUERY_LOG_DISABLE=1 still forces it off (back-compat, wins).
    if os.environ.get("KG_QUERY_LOG_DISABLE", "").lower() in ("1", "true", "yes"):
        return None
    override = os.environ.get("KG_QUERY_LOG", "").strip()
    if override:
        return Path(override).expanduser()
    if os.environ.get("KG_QUERY_LOG_ENABLE", "").lower() in ("1", "true", "yes"):
        return Path.home() / ".cache" / "kg-queries.log"
    return None


def _log_responses() -> bool:
    return os.environ.get("KG_QUERY_LOG_RESPONSES", "").lower() in ("1", "true", "yes")
