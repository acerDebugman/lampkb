...
from kglib.paths import KG_OUT, out_path
...
# Resource cap for parsing untrusted PDF files (F2). A corpus is
# attacker-controllable (kg runs on cloned/shared folders), so screen the
# on-disk size before any parser touches it. Upstream also capped office
# (.docx/.xlsx) zip containers; kg drops office parsing entirely.
_PDF_MAX_RAW_BYTES = 50 * 1024 * 1024            # 50 MiB on-disk


def _file_within_size_cap(path: Path, cap: int = _PDF_MAX_RAW_BYTES) -> bool:
...
def classify_file(path: Path) -> FileType | None:
    ...
    # Office (.docx/.xlsx) and Google Workspace shortcuts are NOT classified:
    # kg is documents-only and never parses them (upstream converts them to
    # markdown sidecars first).
...
def _parse_gitignore_line(raw: str) -> str:
    """Parse one raw line from a .kgignore file per gitignore spec.
...
def _read_ignore_text(path: Path) -> str:
    ...
    for enc in (fallback, "latin-1"):
        ...
        if key not in _warned_ignore_encodings:
            ...
            print(
                f"[kg] WARNING: {path} is not valid UTF-8; read it as "
...
def _load_kgignore(root: Path, *, gitignore: bool = True) -> list[tuple[Path, str]]:
    """Read .kgignore files and return (anchor_dir, pattern) pairs.
...
def _is_ignored(
    ...
    *,
    ...
    """Return True if the path should be ignored per .kgignore patterns.
...
def ignored_predicate(
    ...
    *,
    ...
    """
    root = root.resolve()
    patterns = _load_kgignore(root, gitignore=gitignore)
    explicit_patterns = _load_kgignore(root, gitignore=False)
    # Only shell out to git when .gitignore actually contributes patterns beyond
    # the explicit (.kgignore/--exclude) set: with no .gitignore in play,
    ...
    cache: dict[Path, bool] = {}
    explicit_cache: dict[Path, bool] = {}
    # root's own ignore file is the last entry of _load_kgignore's chain.
...
def detect(root: Path, *, follow_symlinks: bool | None = None, extra_excludes: list[str] | None = None, cache_root: Path | None = None, gitignore: bool = True) -> dict:
    root = root.resolve()
    configured_out_dir = root / KG_OUT
    ...
    if (root / ".kginclude").is_file():
        import sys as _sys
        print(
            "[kg] WARNING: .kginclude is no longer supported "
    ...
    ignore_patterns = _load_kgignore(root, gitignore=gitignore)
    explicit_ignore_patterns = _load_kgignore(root, gitignore=False)
    ...
    # Always include kg-out/memory/ - query results filed back into the graph
    memory_dir = root / KG_OUT / "memory"
    scan_paths = [root]
    ...
    def _on_walk_error(err: OSError) -> None:
        ...
        print(
            f"[kg] WARNING: could not scan {target} ({err}); "
    ...
    for scan_root in scan_paths:
        ...
        for dirpath, dirnames, filenames in os.walk(
            ...
            if not in_memory_tree:
                # dp == root was already loaded by _load_kgignore (root is
    ...
    for p in all_files:
        ...
        if not ftype:
            ...
            continue
        if ftype:
            # Office (.docx/.xlsx) and Google Workspace files never reach here:
            # classify_file no longer classifies them (documents-only).
    ...
    return {
        ...
        "kgignore_patterns": len(ignore_patterns),
...
def load_manifest(
    ...
    *,
    ...
    manifests with absolute keys pass through unchanged, so a kg-out/
